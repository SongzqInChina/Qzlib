...
class Win32Window:
    ...
    def pos(self, hWndInsertAfter=0, x=0, y=0, cx=0, cy=0, uFlags=0):
        return win32gui.SetWindowPos(self._hwnd, hWndInsertAfter, x, y, cx, cy, uFlags)
    ...
    def __getattribute__(self, item):
        if self.hwnd not in handles():
            raise f"HWND {self.hwnd} is not valid"
        else:
